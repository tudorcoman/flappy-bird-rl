        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                ...
                if not jumping:
                    jumping = True
                    start_jump_y = bird_y
                    bird_y_change = -0.6  # Jumping upwards
        if event.type == pygame.KEYUP:
            if event.key == pygame.K_SPACE:
                jumping = False

    if jumping:
        if start_jump_y - bird_y > max_jump_height:
            jumping = False  # Stop the jump if max height is reached
        else:
            bird_y += bird_y_change
    else:
        bird_y_change = 0.2  # Falling down when not jumping
